...
# Training the model:
    def start_train(self, inputs, targets):
        ...
        with tf.Session() as sess:
            ...
            prob_output = self.decoder.decoder(encoder_output, encoder_attention)
            loss_fxn = tf.reduce_mean(self.loss(labels = self.target_pl, logits = prob_output))
            ...
            for i in range(self.epochs):
                ...
                for batch in index:
                    ...
                    _, loss_val = sess.run([min_op, loss_fxn], feed_dict = { self.encoder.X : input_x, self.decoder.input_x : decoder_input, self.target_pl : target})
